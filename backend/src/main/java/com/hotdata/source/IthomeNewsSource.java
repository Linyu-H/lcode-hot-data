...
@Component
public class IthomeNewsSource implements HotSource {
    private final HttpClientFactory http;
    private final ObjectMapper mapper = new ObjectMapper();
    public IthomeNewsSource(HttpClientFactory http) { this.http = http; }
    @Override public String platform() { return "ithome_news"; }
    @Override public String platformName() { return "IT之家资讯"; }
    ...
    public List<HotItem> fetch() throws Exception {
        String body = http.newClient("https://www.ithome.com/").get()
                .uri("https://api.ithome.com/json/newslist/news")
                .retrieve()
                .bodyToMono(String.class)
                .block(http.timeout());
        if (body == null || body.isBlank()) return List.of();
        JsonNode root = mapper.readTree(body);
        JsonNode arr = root.path("newslist");
        if (!arr.isArray() || arr.isEmpty()) arr = root.path("toplist");
