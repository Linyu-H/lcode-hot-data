...
@Component
public class LinuxDoHotSource implements HotSource {

    private static final Logger log = LoggerFactory.getLogger(LinuxDoHotSource.class);
    private final HttpClientFactory http;
    private final FlareSolverrClient flareSolverr;
    private final ObjectMapper mapper = new ObjectMapper();

    public LinuxDoHotSource(HttpClientFactory http, FlareSolverrClient flareSolverr) {
        this.http = http;
    ...
    public List<HotItem> fetch() throws Exception {
        ...
        if (flareSolverr.isEnabled() && flareSolverr.isAvailable()) {
            log.info("Using FlareSolverr to bypass Cloudflare for LinuxDo Hot");
            try {
